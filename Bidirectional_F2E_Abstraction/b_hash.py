...
def expandlevel(initial_matrix,incremented_dir):
    ...
    try:
        ...
        while(len(expandable_list) != 0):
            ...
            if(flim >= h_abstraction(initial_matrix,node.getmatrix(),node.getdirection()) + node.getvalue()):
                #print("5")
...
def h_abstraction(initial_matrix,current_matrix,dir):
    if(True):
        ...
        #print(initial_matrix,current_matrix)
        if dir == 'fw':
            h = blind_for_abstraction.b_hash_abs(current_matrix_temp,goal_matrix)
            blind_for_abstraction.flim = 1
            blind_for_abstraction.nodes_ab[:] = []
            #print(h)
            #print(current_matrix,goal_matrix)
            #print(h)
            return h
        else:
            h = blind_for_abstraction.b_hash_abs(current_matrix_temp,initial_matrix_temp)
            blind_for_abstraction.flim = 1
            blind_for_abstraction.nodes_ab[:] = []
            #print(h)
            #print(current_matrix,initial_matrix)
            #print(h)
            return h
    return 0
